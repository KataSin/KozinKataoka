...
void CameraActor::Default()
{
	...
	//ベクトルを足し算
	rotateUp += vec.x*CameraSpeed*Time::GetInstance().deltaTime();
	rotateLeft -= vec.y*CameraSpeed*Time::GetInstance().deltaTime();
	//キーボード処理
	if (Keyboard::GetInstance().KeyStateDown(KEYCODE::LEFT))
	{
		rotateUp += CameraSpeed*Time::GetInstance().deltaTime();
	}
	...
	{
		rotateLeft += CameraSpeed*Time::GetInstance().deltaTime();
	}
...
void CameraActor::KillCmaera()
{
	...
	//倒された相手の回りをくるくると
	killRotateY += 70.0f*Time::GetInstance().deltaTime();
